...
{
    private static final int UNSET_Y = -32768;
    private Main plugin;
    private String name;
    ...
    public GameConfig( Main plugin, String name, int configVersion )
    {
        this.plugin = plugin;
        this.name = name;
        ...
        if( this.gameType == null ) {
            if( this.ceilingLevel != UNSET_Y && this.groundLevel != UNSET_Y && !this.returnToStartAtGround ) this.gameType = GameType.ELIMINATION_RACE;
            else if( this.ceilingLevel != UNSET_Y ) this.gameType = GameType.RACE;
        ...
        if( gameStartsSection != null ) {
            for( String startSpawnNode : gameStartsSection.getKeys( false ) ) {
                ...
                if( spawn != null ) {
                    if( spawn.getBlockY() <= this.groundLevel && this.groundLevel != UNSET_Y ) {
                    ...
                    if( spawn.getBlockY() >= this.ceilingLevel && this.ceilingLevel != UNSET_Y ) {
        ...
        switch( this.gameType ) {
            case ELIMINATION:
                if( this.groundLevel == UNSET_Y ) plugin.getLogger().warning( "Ground must be set for Elimination type game " + this.name );
            ...
            case ELIMINATION_RACE:
                if( this.groundLevel == UNSET_Y ) plugin.getLogger().warning( "Ground must be set for Elimination Race type game " + this.name );
                if( this.ceilingLevel == UNSET_Y && this.winBlock == null ) plugin.getLogger().warning( "Ceiling or win block must be set for Elimination Race type game " + this.name );
                ...
                if( this.groundLevel >= this.ceilingLevel && this.groundLevel != UNSET_Y && this.ceilingLevel != UNSET_Y ) {
                    plugin.getLogger().warning( "Ceiling " + this.ceilingLevel + " should be higher than ground " + this.groundLevel + " for game " + this.name );
                }
                break;
            case RACE:
                if( this.ceilingLevel == UNSET_Y && this.winBlock == null ) plugin.getLogger().warning( "Ceiling or win block must be set for Race type game " + this.name );
                if( this.returnToStartAtGround && this.groundLevel == UNSET_Y ) {
                    plugin.getLogger().warning( "Ground is required since Return to Start is true for Race type game " + this.name );
                }
                if( this.returnToStartAtGround && this.groundLevel >= this.ceilingLevel && this.groundLevel != UNSET_Y && this.ceilingLevel != UNSET_Y ) {
                    plugin.getLogger().warning( "Ceiling " + this.ceilingLevel + " should be higher than ground " + this.groundLevel + " for game " + this.name );
                }
                break;
            case PARKOUR:
                if( this.ceilingLevel == UNSET_Y && this.winBlock == null ) plugin.getLogger().warning( "Ceiling or win block must be set for Parkour type game " + this.name );
                if( this.returnToStartAtGround && this.groundLevel == UNSET_Y ) {
    ...
    public GameConfig( Main plugin, String name )
    {
        ...
        this.groundLevel = 64;
        this.ceilingLevel = UNSET_Y;
        this.returnToStartAtGround = false;
        ...
        this.countdownAnnounceTime = 10;
        this.winCommands = null;
    }
    ...
    public boolean hasCeilingLevel()
    {
        return ceilingLevel != UNSET_Y;
    }
    ...
    public int getStartSpawnsMinY() {
        if( startSpawns.isEmpty() ) return UNSET_Y;
    ...
    public boolean isProperlyConfigured(){
        ...
        {
            case ELIMINATION:
                if( this.getGroundLevel() == UNSET_Y ) return false;
                if( this.getLossSpawn() == null ) return false;
                if( this.getStartSpawnsMinY() <= this.getGroundLevel() ) return false;
                break;
            case ELIMINATION_RACE:
                if( this.getGroundLevel() == UNSET_Y ) return false;
                if( this.getCeilingLevel() == UNSET_Y && this.getWinBlock() == null ) return false;
                if( this.getLossSpawn() == null ) return false;
                if( this.getGroundLevel() >= this.getCeilingLevel() && this.getCeilingLevel() != UNSET_Y ) return false;
                if( this.getStartSpawnsMinY() <= this.getGroundLevel() ) return false;
                if( this.getStartSpawnsMaxY() >= this.getCeilingLevel() && this.getCeilingLevel() != UNSET_Y ) return false;
                break;
            case RACE:
            case PARKOUR:
                if( this.getCeilingLevel() == UNSET_Y && this.getWinBlock() == null ) return false;
    ...
    {
        boolean errorCeilingOrWinBlock = ( this.winBlock == null && this.ceilingLevel == UNSET_Y && this.gameType != GameType.ELIMINATION );
        ...
        if( this.startSpawns.isEmpty() ) {
            ...
            for( SpawnLocation startSpawn : this.startSpawns ) {
                Location spawn = startSpawn.getConfiguredSpawn();
                boolean errorStartToGround = ( spawn.getBlockY() <= this.groundLevel && this.groundLevel != UNSET_Y );
                boolean errorStartToCeiling = ( spawn.getBlockY() >= this.ceilingLevel && this.ceilingLevel != UNSET_Y );
